...
public class TareaDAO {
    ...
    public List<Tarea> findAll() throws SQLException {
        ...
        try (Statement stmt = getConn().createStatement();
             ResultSet rs = stmt.executeQuery("SELECT * FROM tareas ORDER BY CASE prioridad WHEN 'Alta' THEN 1 WHEN 'Media' THEN 2 ELSE 3 END, fecha_limite")) {
            while (rs.next()) list.add(map(rs));
        }
        return list;
    }

    public List<Tarea> findByEstado(String estado) throws SQLException {
        List<Tarea> list = new ArrayList<>();
        try (PreparedStatement ps = getConn().prepareStatement("SELECT * FROM tareas WHERE estado=? ORDER BY fecha_limite")) {
    ...
    public void insert(Tarea t) throws SQLException {
        String sql = "INSERT INTO tareas (titulo, descripcion, prioridad, estado, fecha_limite, categoria) VALUES (?,?,?,?,?,?)";
