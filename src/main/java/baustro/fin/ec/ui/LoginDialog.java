...
public class LoginDialog extends JDialog {
    ...
    private void buildUI() {
        ...
        JLabel lblPw = fieldLabel(firstTime ? "Nueva contrasena:" : "Contrasena:");
        JLabel lblConfirm = fieldLabel("Confirmar contrasena:");
        ...
        JButton btnLogin = StyledComponents.primaryButton(
                firstTime ? "Crear contrasena y entrar" : "Entrar",
                UIConstants.ACCENT_BLUE);
        btnLogin.setPreferredSize(new Dimension(300, 40));

        JLabel hint = new JLabel("Esta contrasena protege el acceso a la app", SwingConstants.CENTER);
        hint.setFont(UIConstants.FONT_SMALL);
        hint.setForeground(UIConstants.TEXT_MUTED);

        Runnable doLogin = () -> {
            String pw = new String(pwField.getPassword());
            if (pw.isEmpty()) { errorLabel.setText("Ingresa tu contrasena."); return; }

            if (firstTime) {
                String confirm = new String(pwConfirm.getPassword());
                if (!pw.equals(confirm)) { errorLabel.setText("Las contrasenas no coinciden."); return; }
                if (pw.length() < 4) { errorLabel.setText("Minimo 4 caracteres."); return; }
                try {
                    saveHash(pw);
