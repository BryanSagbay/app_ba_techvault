...
public class CorrectivoPanel extends JPanel {
    ...
    private List<Correctivo> allData = new ArrayList<>();
    ...
    private void buildUI() {
        ...
        JLabel title = new JLabel("Correctivos");
        ...
        hsf = new HeaderSearchFilter(
                ...
                new HeaderSearchFilter.ComboConfig("Ordenar",   new String[]{
                        "Fecha reciente","Fecha antigua","Prioridad Alta","Estado","N Tarea","Servicio"}, "Fecha reciente")
        ...
        bottom.add(btnView); bottom.add(btnEdit); bottom.add(btnDelete);
    ...
    private void applyFilters() {
        ...
        Stream<Correctivo> s = allData.stream();
        ...
        Comparator<Correctivo> cmp = switch (sort) {
            ...
            default                -> Comparator.comparing((Correctivo c) -> nvl(c.getFechaReporte())).reversed();
        };
        List<Correctivo> result = s.sorted(cmp).toList();
        refreshTable(result);
        updateStats(result);
    }

    private boolean matches(Correctivo c, String q) {
        return nv(c.getNumeroTarea(),q)||nv(c.getTitulo(),q)||nv(c.getServicio(),q)
              ||nv(c.getErrorPresentado(),q)||nv(c.getSolucion(),q)||nv(c.getAmbiente(),q);
    }
    ...
    private void refreshTable(List<Correctivo> data) {
        ...
        for (Correctivo c : data)
            tableModel.addRow(new Object[]{i++,c.getNumeroTarea(),c.getTitulo(),
                c.getServicio(),c.getAmbiente(),c.getPrioridad(),c.getEstado(),c.getFechaReporte()});
    }

    private void updateStats(List<Correctivo> data) {
    ...
    private void editSelected()   { Correctivo c=getSelected(); if(c!=null) openForm(c); }
    private void viewSelected()   { Correctivo c=getSelected(); if(c!=null) showDetail(c); }
    private void deleteSelected() {
        Correctivo c=getSelected(); if(c==null) return;
        int ok=JOptionPane.showConfirmDialog(this,"Eliminar ["+c.getNumeroTarea()+"] "+c.getTitulo()+"?",
                "Confirmar",JOptionPane.YES_NO_OPTION,JOptionPane.WARNING_MESSAGE);
        if(ok==JOptionPane.YES_OPTION){try{dao.delete(c.getId());loadData();}
        catch(Exception ex){JOptionPane.showMessageDialog(this,"Error: "+ex.getMessage());}}
    }

    private void showDetail(Correctivo c) {
    ...
    private void openForm(Correctivo existing) {
        ...
        JTextField fTit   = StyledComponents.styledTextField("Descripcion breve");
        ...
        addRow(form,gbc,r++,"Fecha Reporte",fFR,"Fecha Solucion",fFS);
        addRow(form,gbc,r++,"Responsable",fResp,null,null);
        addFull(form,gbc,r++,"Descripcion",new JScrollPane(fDesc));
        addFull(form,gbc,r++,"Error presentado",new JScrollPane(fErr));
        addFull(form,gbc,r++,"Solucion aplicada",new JScrollPane(fSol));
