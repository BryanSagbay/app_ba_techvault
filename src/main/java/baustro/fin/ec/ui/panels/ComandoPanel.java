...
public class ComandoPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        hsf = new HeaderSearchFilter(
                "Buscar comando, descripcion...",
                new HeaderSearchFilter.ComboConfig("SO",       UIConstants.SISTEMAS_OPERATIVOS_CMD, "Todos"),
                new HeaderSearchFilter.ComboConfig("Categoria",new String[]{"Red","Logs","Deploy","BD","Sistema","Docker","Git","Otro"}, "Todas"),
                new HeaderSearchFilter.ComboConfig("Ordenar",  new String[]{"Titulo A-Z","Categoria","SO"}, "Titulo A-Z")
        ...
        add(header,BorderLayout.NORTH);add(split,BorderLayout.CENTER);add(bottom,BorderLayout.SOUTH);
    }
    ...
    private void applyFilters(){
        ...
        String so  =hsf.getFilter(0);String cat=hsf.getFilter(1);String sort=hsf.getFilter(2);
        Stream<Comando> s=allData.stream();
        if(!q.isEmpty())  s=s.filter(c->nv(c.getTitulo(),q)||nv(c.getComando(),q)||nv(c.getDescripcion(),q));
    ...
    private void openForm(Comando existing){
        ...
        bS.addActionListener(e->{
            if(fTit.getText().trim().isEmpty()||fCmd.getText().trim().isEmpty()){JOptionPane.showMessageDialog(d,"Titulo y comando obligatorios.");return;}
            Comando c=existing!=null?existing:new Comando();
            c.setTitulo(fTit.getText().trim());c.setCategoria(fCat.getText().trim());c.setSistemaOperativo((String)fSO.getSelectedItem());c.setComando(fCmd.getText().trim());c.setDescripcion(fDesc.getText().trim());
